...
function buildSystemPrompt({ name, description, planContent, specContent }) {
    ...
    sections.push(
        ...
        '  - `escalate_to_human` — 升级人工询问。带 question / choices / context_files / blocking。',
        '    · blocking=true：必须等用户当面拍板，前端弹出阻塞式对话框（用户必选一项或填一句才能继续）——即使在全自治模式也会弹，仅在「不拿到决定就无法继续」时设；带 choices 会被自动视为 blocking。',
        '    · 不设 blocking（且无 choices）：全自治模式下 alias 成 record_alert(C2) 走非阻塞提示，用户可事后再看。',
